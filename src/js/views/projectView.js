    ...
    displayMedia() {
        this.mediaBtnArr.forEach((cur) => {
            cur.addEventListener('click', () => {
                ...
                iconSelect.style.fill = red;

                // Hide everything, to then show the selected one later
                this.hideMedia();
...
export function projectSetUp(){
    ...
    projectContArr.forEach((cur) => {
        ...
        projectArray.push(projectView);
    });

    for (let i=0; i < galleryBtnArr.length; i++) {
        projectMap.set(galleryBtnArr[i], projectArray[i])
    }
  }
